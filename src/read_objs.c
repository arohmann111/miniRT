...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/15 15:49:46 by arohmann          #+#    #+#             */
/*   Updated: 2022/06/15 15:58:50 by arohmann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_plane(t_scene *scene, char **split, int line_cnt)
{
	...
	if (get_vector(&((t_object *)new_elem->content)->pos,
			split[1], COORDINATES, line_cnt) == ERROR)
		return (error_free(ERROR, split));
	if (get_vector(&((t_object *)new_elem->content)->pl.orient,
			split[2], ORIENTATION, line_cnt) == ERROR)
		return (error_free(ERROR, split));
	if (get_colors(&((t_object *)new_elem->content)->colors,
			split[3], line_cnt) == ERROR)
		return (error_free(ERROR, split));
	if (get_material((t_object *)new_elem->content,
			split[4], line_cnt) == ERROR)
		return (error_free(ERROR, split));
	ft_free_array(split);
	return (0);
}
